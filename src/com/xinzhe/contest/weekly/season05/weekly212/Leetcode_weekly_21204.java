 ...
 * @date 2020/10/25
 * Title : 1632. 矩阵转换后的秩
 * Description : 给你一个  m x n  的矩阵 matrix  ，请你返回一个新的矩阵  answer  ，其中  answer[row][col]  是  matrix[row][col]  的秩。
 *              每个元素的  秩  是一个整数，表示这个元素相对于其他元素的大小关系，它按照如下规则计算：
 *              如果一个元素是它所在行和列的最小值，那么它的 秩  为 1。如果两个元素  p 和  q  在 同一行  或者 同一列  ，那么：
 *              如果  p < q ，那么  rank(p) < rank(q)如果  p == q  ，那么  rank(p) == rank(q)如果  p > q  ，那么  rank(p) > rank(q)
 *              秩  需要越 小  越好。题目保证按照上面规则  answer  数组是唯一的
 * link : https://leetcode-cn.com/problems/rank-transform-of-a-matrix
 * Level : Hard
 */

//todo undo
public class Leetcode_weekly_21204 {
    public static void main(String[] args) {
        Leetcode_weekly_21204 lc = new Leetcode_weekly_21204();
        int[] arr = {3,5};
    }
    int[] p;//并查集，用于合并相同大小的元素，保证相同大小的元素秩相同，且应为这些相同元素中秩的最大值
    int[] values;//对应下标的秩的值(下标使用indexes数组中的下标值表示)
    ...
    public int[][] matrixRankTransform(int[][] matrix) {
        ...
        //按照矩阵中的值排序indexes
        Arrays.sort(indexes, (Integer i, Integer j) -> (matrix[i / col][i % col] - matrix[j / col][j % col]));
        ...
        //将values中每个元素的秩转化到二维矩阵返回
        int[][] res = new int[row][col];
        for(int i = 0; i < row; i++){
            for(int j = 0; j < col; j++){
                int idx = i * col + j;
                res[i][j] = values[find(idx)];
            }
        }
        return res;
    }
